...
namespace Crawler.LinkScraper
{
    public unsafe class FastLinkScraper: ILinkScraper
    {
        ...
        public IReadOnlyList<Uri> GetLinks(Uri parentUri, string pageHtml)
        {
                    ...
                    else if (insideTag)
                    {
                        ...
                        if (insideLink)
                        {
                            ...
                            if (!string.IsNullOrEmpty(uri))
                            {
                                try
                                {
                                    uris.Add(new Uri(parentUri, uri));
                                }
                                catch (UriFormatException ex)
                                {
                                    s_logger.Warn("Failed to parse uri {0} - {1}", uri, ex.Message);
                                }
                            }
